...
- Running the README demo commands yourself writes NEW run folders here —
  that is by design (every run leaves evidence). `git status` will show
  them as untracked; remove with `git clean -fd evidence/` when done.
