...
| `replay/20260819-201218-msr5/` | **Cross-tenant reuse**: the same artifact through `tenants/summit-fcu.overlay.json` — two additive `prependStrategy` patches, nothing else | `success` against the re-skinned tenant, every step at strategy 0 via the tenant's locators — record once, reuse per tenant via a reviewed overlay, no re-recording |
